...
use crate::dspy::{
    ...
    multimodal::{MultiModalInput, MultiModalOutput, MediaType},
...
impl ImageTextAlignmentMetric {
    ...
    fn calculate_semantic_alignment(&self, text: &str, image_description: &str) -> f64 {
        ...
        let common_words = text_words.iter()
    ...
    fn calculate_visual_alignment(&self, _text: &str, _image_features: &HashMap<String, f64>) -> f64 {
...
impl Metric<MultiModalInput, MultiModalOutput> for ImageTextAlignmentMetric {
    ...
    fn evaluate(&self, example: &Example<MultiModalInput, MultiModalOutput>, prediction: &MultiModalOutput) -> DspyResult<MetricResult> {
        ...
        let image_description = output.media_analysis
            .values()
            .find_map(|analysis| {
        ...
        // Combine scores
        let overall_score = (semantic_score * self.weight_semantic) + 
                           (visual_score * self.weight_visual);

        let passed = overall_score >= self.threshold;

        let mut details = HashMap::new();
        details.insert("semantic_score".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(semantic_score).unwrap()));
        details.insert("visual_score".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(visual_score).unwrap()));
        details.insert("threshold".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(self.threshold).unwrap()));
...
impl VisualQuestionAccuracyMetric {
    ...
    fn calculate_accuracy(&self, predicted: &str, expected: &str) -> f64 {
        ...
        let common_words = pred_words.iter()
...
impl Metric<VisionInput, VisionOutput> for VisualQuestionAccuracyMetric {
    ...
    fn evaluate(&self, example: &Example<VisionInput, VisionOutput>, prediction: &VisionOutput) -> DspyResult<MetricResult> {
        let input = &example.input;
        let output = prediction;
        // For evaluation, we need expected answer in metadata
        let expected_answer = input.metadata
            .get("expected_answer")
            .and_then(|v| v.as_str())
            .ok_or_else(|| DspyError::invalid_input(
                "Expected answer not found in input metadata"
            ))?;
        ...
        let mut details = HashMap::new();
        details.insert("accuracy".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(accuracy).unwrap()));
        details.insert("threshold".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(self.threshold).unwrap()));
        details.insert("predicted_answer".to_string(), serde_json::Value::String(output.analysis.clone()));
        details.insert("expected_answer".to_string(), serde_json::Value::String(expected_answer.to_string()));
...
impl Metric<MultiModalInput, MultiModalOutput> for CrossModalConsistencyMetric {
    ...
    fn evaluate(&self, example: &Example<MultiModalInput, MultiModalOutput>, prediction: &MultiModalOutput) -> DspyResult<MetricResult> {
        ...
        let mut details = HashMap::new();
        details.insert("consistency_score".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(consistency).unwrap()));
        details.insert("threshold".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(self.threshold).unwrap()));
        details.insert("media_count".to_string(), serde_json::Value::Number(serde_json::Number::from(input.media.len())));
...
mod tests {
    ...
    fn test_image_text_alignment_metric() {
        ...
        let input = MultiModalInput::new("A cat sitting on a chair".to_string())
            .add_media(image);
    ...
    fn test_cross_modal_consistency_metric() {
        ...
        let image = create_test_image();
        let input = MultiModalInput::new("Show me a cat".to_string())
            .add_media(image);

...
impl MediaContent {
    ...
    pub async fn from_file<P: AsRef<Path>>(path: P) -> DspyResult<Self> {
        ...
        metadata.insert("file_path".to_string(), serde_json::Value::String(path.display().to_string()));
        metadata.insert("file_size".to_string(), serde_json::Value::Number(data.len().into()));
    ...
    pub fn from_bytes(data: Vec<u8>, mime_type: String) -> DspyResult<Self> {
        let media_type = Self::media_type_from_mime(&mime_type)?;
        let mut metadata = HashMap::new();
        metadata.insert("data_size".to_string(), serde_json::Value::Number(data.len().into()));
    ...
    fn detect_mime_type(path: &Path) -> DspyResult<String> {
        let extension = path.extension()
            ...
            .ok_or_else(|| DspyError::configuration("file_extension", "Could not determine file extension"))?;
        ...
        let mime_type = match extension.to_lowercase().as_str() {
            ...
            "pdf" => "application/pdf",
            "txt" => "text/plain",
            _ => return Err(DspyError::configuration("mime_type", &format!("Unsupported file extension: {}", extension))),
        };
    ...
    fn media_type_from_mime(mime_type: &str) -> DspyResult<MediaType> {
        let media_type = if mime_type.starts_with("image/") {
            ...
            return Err(DspyError::configuration("media_type", &format!("Unsupported MIME type: {}", mime_type)));
        };
...
impl MultiModalInput {
    ...
    pub fn get_media_by_type(&self, media_type: &MediaType) -> Vec<&MediaContent> {
        self.media.iter().filter(|m| &m.media_type == media_type).collect()
    }
}
    ...
    async fn validate_media(&self, media: &MediaContent) -> DspyResult<()> {
        ...
        if size_mb > self.config.max_media_size_mb as f64 {
            return Err(DspyError::module(
                &self.name,
                &format!("Media size {:.1}MB exceeds limit of {}MB", size_mb, self.config.max_media_size_mb)
            ));
        }
    ...
    async fn process_multimodal(&self, input: MultiModalInput) -> DspyResult<MultiModalOutput> {
        ...
        for (key, analysis) in &media_analyses {
            ...
            if let Some(transcription) = analysis.get("transcription").and_then(|v| v.as_str()) {
                prompt_parts.push(format!("Audio transcription for {}: {}", key, transcription));
            }
        }
        ...
        output = output.add_metadata("media_count".to_string(), serde_json::Value::Number(input.media.len().into()));
        output = output.add_metadata("has_images".to_string(), serde_json::Value::Bool(input.has_media_type(&MediaType::Image)));
        output = output.add_metadata("has_audio".to_string(), serde_json::Value::Bool(input.has_media_type(&MediaType::Audio)));
...
impl Module for MultiModalPredict {
    ...
    async fn validate_input(&self, input: &Self::Input) -> DspyResult<()> {
        if input.text.trim().is_empty() && input.media.is_empty() {
            return Err(DspyError::module(self.name(), "Input must contain either text or media"));
        }
    ...
    async fn validate_output(&self, output: &Self::Output) -> DspyResult<()> {
        if output.text.trim().is_empty() {
            return Err(DspyError::module(self.name(), "Output text cannot be empty"));
        }
    ...
    async fn forward(&self, mut input: Self::Input) -> DspyResult<Self::Output> {
        info!("Processing multi-modal input with {} media items", input.media.len());
...
mod tests {
    ...
    async fn test_multimodal_input_creation() {
        ...
        assert_eq!(input.metadata.get("key").unwrap(), &serde_json::Value::String("value".to_string()));
    }
    ...
    async fn test_media_validation() {
        ...
        // Test valid media
        let valid_media = MediaContent::from_bytes(
            b"small image".to_vec(),
            "image/jpeg".to_string()
        ).unwrap();
    ...
    async fn test_multimodal_processing() {
        ...
        let media = MediaContent::from_bytes(
            b"test image data".to_vec(),
            "image/jpeg".to_string()
        ).unwrap();
        
        let input = MultiModalInput::new("Describe this image".to_string())
            .add_media(media);

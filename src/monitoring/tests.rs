...
async fn test_sampling_config_default() {
    ...
    assert!(config.high_frequency_metrics.contains(&"cpu_usage".to_string()));
}
...
async fn test_metric_value_types() {
    ...
    let histogram = MetricValue::Histogram(HistogramData {
        buckets: vec![HistogramBucket { upper_bound: 1.0, count: 10 }],
        count: 10,
        sum: 5.0,
    });
    let summary = MetricValue::Summary(SummaryData {
        quantiles: vec![Quantile { quantile: 0.5, value: 2.5 }],
        count: 20,
        sum: 50.0,
    });
...
async fn test_http_metrics_collector_record_request() {
    ...
    collector.record_request("/api/test", 200, 150.0).await.unwrap();

...
use super::audit::*;
use super::session::*;
use super::policy::{self, PolicyEngine, SimplePolicyEngine, SecurityPolicy};
...
async fn test_user_authentication() {
    ...
    let wrong_auth_result = service.authenticate(&user_data.username, "WrongPassword").await;
...
async fn test_authorization_permission_check() {
    ...
    let denied_result = service.check_permission(&context, "document", "delete").await;
...
async fn test_encryption_service() {
    ...
    // Generate key
    let gen_result = service.generate_key(key_id, EncryptionAlgorithm::Aes256Gcm).await;
...
async fn test_encryption_string_operations() {
    ...
    // Generate key
    service.generate_key(key_id, EncryptionAlgorithm::Aes256Gcm).await.unwrap();
...
async fn test_session_data_operations() {
    ...
    // Set session data
    let set_result = manager.set_session_data(&session_id, "key1", "value1".to_string()).await;
...
async fn test_audit_service() {
    ...
    assert!(matches!(logs[0].event, SecurityEvent::AuthenticationAttempt { .. }));
}
...
async fn test_policy_engine() {
    ...
    let context = create_test_security_context();
    let eval_result = engine.evaluate_all_policies(&context, "test_resource", "read").await;
...
async fn test_rate_limiting_service() {
    ...
    // Record the request
    service.record_request(key, limit_type.clone()).await.unwrap();
...
async fn test_rate_limiting_enforcement() {
    ...
    for i in 1..=2 {
        let result = service.check_rate_limit(key, limit_type.clone()).await.unwrap();
...
async fn test_rate_limiting_statistics() {
    ...
    // Make some requests
    for _ in 0..3 {
        service.check_rate_limit(key, limit_type.clone()).await.unwrap();
        service.record_request(key, limit_type.clone()).await.unwrap();
    }
...
async fn test_security_middleware_authentication_required() {
    ...
    let result = result.unwrap();
    assert!(!result.allowed);
    assert_eq!(result.denial_reason, Some("Authentication required".to_string()));
}
...
async fn test_security_middleware_request_size_limit() {
    ...
    let result = result.unwrap();
    assert!(!result.allowed);
    assert!(result.denial_reason.as_ref().unwrap().contains("Request body too large"));
}

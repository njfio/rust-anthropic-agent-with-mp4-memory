...
async fn test_multimodal_input_creation() {
    ...
    let input = MultiModalInput::new("Analyze this content".to_string())
        ...
        .with_metadata("test_key".to_string(), serde_json::Value::String("test_value".to_string()));
...
async fn test_multimodal_output_creation() {
    let output = MultiModalOutput::new("Analysis result".to_string(), 0.9)
        .add_media_analysis("image_0".to_string(), serde_json::json!({
            "objects": ["person", "car"],
            "scene": "street"
        }))
        .add_metadata("processing_time".to_string(), serde_json::Value::Number(150.into()));
...
async fn test_multimodal_predict_creation() {
    ...
    let signature = Signature::<MultiModalInput, MultiModalOutput>::new("test_multimodal".to_string())
...
async fn test_multimodal_input_validation() {
    ...
    // Test valid input
    let image = create_test_image();
    let valid_input = MultiModalInput::new("Test query".to_string())
        .add_media(image);
    ...
    let oversized_input = MultiModalInput::new("Test".to_string())
        .add_media(large_image);
...
async fn test_vision_output_creation() {
    ...
    let output = VisionOutput::new("I see a person in the image".to_string(), 0.9)
        ...
        .add_metadata("model".to_string(), serde_json::Value::String("claude-3-vision".to_string()));
    
    assert_eq!(output.analysis, "I see a person in the image");
    assert_eq!(output.confidence, 0.9);
    assert_eq!(output.detected_objects.len(), 1);
    assert_eq!(output.extracted_text, Some("Some text in the image".to_string()));
...
async fn test_vision_input_validation() {
    ...
    // Test unsupported format
    let unsupported_image = MediaContent::from_bytes(
        b"test".to_vec(),
        "image/bmp".to_string()
    ).unwrap();
...
async fn test_multimodal_integration() {
    ...
    let mm_signature = Signature::<MultiModalInput, MultiModalOutput>::new("multimodal_test".to_string());
    ...
    // Multi-modal input
    let mm_input = MultiModalInput::new("Analyze this image".to_string())
        .add_media(image.clone());
...
async fn test_multimodal_error_handling() {
    ...
    let test_cases = vec![
        ...
        (MultiModalInput::new("".to_string()), "Input must contain either text or media"),
        
        // Oversized media
        (MultiModalInput::new("Test".to_string()).add_media(
            MediaContent::from_bytes(vec![0u8; 20 * 1024 * 1024], "image/jpeg".to_string()).unwrap()
        ), "exceeds limit"),
    ...
    for (input, expected_error) in test_cases {
        ...
        assert!(error_msg.contains(expected_error), "Expected '{}' in error: {}", expected_error, error_msg);
    }
}

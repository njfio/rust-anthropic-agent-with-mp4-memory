...
use rust_memvid_agent::cli::dspy::{
    DspyCliConfig, DspyConfigManager, DspyCliError, DspyCliResult,
...
async fn test_dspy_config_save_and_load() {
    ...
    assert!(loaded_config.is_ok(), "Should be able to load configuration");
...
async fn test_config_to_dspy_config_conversion() {
    ...
    assert_eq!(dspy_config.max_optimization_iterations, cli_config.optimization.max_iterations as u32);
}
...
fn test_string_utils_functions() {
    ...
    assert_eq!(
        StringUtils::parse_comma_separated(""),
        Vec::<String>::new()
    );
}
...
async fn test_file_utils_ensure_dir_exists() {
    ...
    let result = FileUtils::ensure_dir_exists(&test_dir).await;
    assert!(result.is_ok(), "Should handle existing directory gracefully");
}

...
async fn test_benchmark_slow_module() {
    ...
    let has_latency_optimization = results.optimization_suggestions.iter()
...
async fn test_benchmark_error_prone_module() {
    ...
    let has_error_optimization = results.optimization_suggestions.iter()
...
async fn test_benchmark_results_export() {
    ...
    let prometheus_export = suite.export_results(BenchmarkOutputFormat::Prometheus).unwrap();
...
async fn test_performance_metrics_calculation() {
    ...
    let total_percentage: f64 = metrics.latency_distribution.iter()
...
async fn test_optimization_suggestions() {
    ...
    let test_inputs = vec!["test".to_string()];
    let results = suite.benchmark_module(&slow_module, test_inputs).await.unwrap();
    
    let suggestions = &results.optimization_suggestions;
    assert!(!suggestions.is_empty());
    
    // Should suggest caching for high latency
    let has_caching_suggestion = suggestions.iter()
        .any(|s| s.category == OptimizationCategory::Caching);
    ...
    let test_inputs = vec!["test".to_string()];
    let results = suite.benchmark_module(&error_module, test_inputs).await.unwrap();
    
    let suggestions = &results.optimization_suggestions;
    
    // Should suggest configuration tuning for high error rate
    let has_config_suggestion = suggestions.iter()
    ...
    let has_critical_suggestion = suggestions.iter()
        .any(|s| s.priority == OptimizationPriority::Critical);
